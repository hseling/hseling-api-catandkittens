...
def write_to_db_metas(data_to_process):
    ...
    for text in data_to_process.values():
        ...
        CUR.execute("INSERT INTO meta_cat (domain, year, author, journal, source, article_name) VALUES (%s, %s, %s, %s, %s, %s)", (metadata[4],metadata[3],metadata[2], metadata[5],metadata[0], metadata[1]))
        CON.commit()
...
def write_to_db_words(data_to_process):
    ...
    for file in data_to_process.values():
        ...
        for line in lines:
            if line != '':
                spl_line = line.split('\t')
                CUR.execute("INSERT INTO words_cat (id_sent, word, lemma, POS_tag, Morph, root_dist, sint_role) VALUES (%s, %s, %s, %s, %s, %s, %s)",(spl_line[0],spl_line[1],spl_line[2], spl_line[3],  spl_line[4] , spl_line[6],  spl_line[7]))
                CON.commit()
        ...
        try:
            ...
            for i, sheetname in enumerate(tqdm(('bigrams', 'trigrams','quadrograms','fivegrams','sixgrams'))):
                ...
                for r in range(1, ngrams.nrows):
                    ...
                    CUR.execute("INSERT INTO "+"collocations_{0}_grams".format(i+2)+" (occurence, POS_tag, absolute_frequency, normalized_frequency, PMI, log_likelihood, t_score) VALUES (%s, %s, %s, %s, %s, %s, %s)",values)
                CON.commit()
